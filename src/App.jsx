...
const initialData = [
    ...
    cards: [
      {
        ...
        description: 'Official API specification and documentation for Java Standard Edition 21 LTS.',
      ...
      },
      {
        ...
        description: 'In-depth, code-driven tutorials covering core Java features and modern APIs.',
      ...
      },
      {
        ...
        description: 'Official repository containing the source code for OpenJDK.',
    ...
    cards: [
      {
        ...
        description: 'Comprehensive guide to building production-ready applications with Spring Boot.',
      ...
      },
      {
        ...
        description: 'Quick-start generator tool for bootstrapping new Spring Boot projects.',
      ...
      },
      {
        ...
        description: 'Source code for the core Spring Framework, IOC Container, and Web MVC.',
  ...
  },
  {
    id: 'sec-github',
    title: 'GitHub & Git',
    description: 'Version control specs, CLI utilities, and repository management',
    cards: [
      {
        id: 'card-7',
        title: 'GitHub Documentation',
        url: 'https://docs.github.com/',
        description: 'Help articles, API guides, Actions workflows, and GitHub ecosystem details.',
        tags: ['Docs', 'Git', 'CI/CD']
      },
      {
        id: 'card-8',
        ...
        description: 'The definitive book on Git version control written by Scott Chacon and Ben Straub.',
        tags: ['Book', 'Guide', 'Git']
      },
      {
        id: 'card-9',
        ...
        description: 'Command line tool manuals for managing GitHub PRs, issues, and repositories from the terminal.',
        tags: ['CLI', 'Tooling']
      }
    ]
  }
...
export default function App() {
  ...
  const handleCardSubmit = (e) => {
    ...
    if (editingCardId) {
      setData(prevData =>
        prevData.map(sec => {
          // Filter out the old card instance
          const filteredCards = sec.cards.filter(c => c.id !== editingCardId);
          // If this is the target section, add updated card
          if (sec.id === sectionId) {
            return {
              ...sec,
              cards: [
                ...filteredCards,
                { id: editingCardId, title, url, description, tags: tagArray }
              ]
            };
          }
          return { ...sec, cards: filteredCards };
        })
      );
        ...
        prevData.map(sec => {
          if (sec.id === sectionId) {
            return {
              ...
              cards: [
                ...sec.cards,
                { id: 'card-' + Date.now(), title, url, description, tags: tagArray }
              ]
            };
          }
          return sec;
  ...
  const deleteSection = (sectionId) => {
    ...
    if (window.confirm(`Delete section "${sec.title}" and all its ${sec.cards.length} cards?`)) {
    ...
    <div className="bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 min-h-screen font-sans antialiased transition-colors duration-200">
      ...
      {toast && (
        <div className="fixed bottom-5 right-5 z-50 pointer-events-none">
          <div className={`px-4 py-2.5 rounded-lg shadow-lg text-xs font-medium flex items-center gap-2 transition-all duration-300 ${
      ...
      <header className="sticky top-0 z-30 border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-slate-900 dark:text-white text-base leading-none">Dev Hub</h1>
              <span class="text-xs text-slate-500 dark:text-slate-400">Learning Knowledge Base</span>
            </div>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="space-y-12">
          {data.map(sec => {
            const filteredCards = sec.cards.filter(card => 
            ...
            return (
              <section key={sec.id} id={sec.id} className="space-y-4 scroll-mt-24">
                ...
                <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-3">
                  <div>
                    <div className="flex items-center gap-2">
                      <h2 className="text-xl font-bold text-slate-900 dark:text-white tracking-tight">{sec.title}</h2>
                    ...
                    >
                      <div>
                        ...
                        <h3 className="font-semibold text-slate-900 dark:text-slate-100 text-sm hover:text-blue-600 dark:hover:text-blue-400 transition-colors line-clamp-1">
                          <a href={card.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5">
                            {card.title}
                            <svg className="w-3.5 h-3.5 text-slate-400 shrink-0 inline" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      ...
                      {card.tags && card.tags.length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-3 pt-2 border-t border-slate-100 dark:border-slate-800/60">
                          {card.tags.map((tag, idx) => (
                            <span key={idx} className="text-[10px] font-medium px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400">
                              #{tag}
                            </span>
                          ))}
                        </div>
        ...
        {totalVisibleCards === 0 && (
          <div className="text-center py-16">
            ...
            <h3 className="text-lg font-semibold text-slate-800 dark:text-slate-200">No resources found</h3>
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Try adjusting your search query or add a new resource card.</p>
          </div>
        )}
      </main>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            ...
            <form onSubmit={handleCardSubmit} className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-1">Target Section</label>
                <select 
              ...
              <div>
                <label className="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-1">URL / Link</label>
                <input 
                  type="url" 
                  value={cardForm.url}
              ...
              <div>
                <label className="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-1">Tags (comma separated)</label>
                <input 
                  type="text" 
                  value={cardForm.tags}
      ...
      {isSectionModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl w-full max-w-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
              <h3 className="font-bold text-slate-900 dark:text-white">Add New Section</h3>
              <button onClick={closeModals} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                </svg>
            ...
            <form onSubmit={handleSectionSubmit} className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-1">Section Title</label>
                <input 
                  type="text" 
              ...
              <div>
                <label className="block text-xs font-semibold uppercase tracking-wider text-slate-500 mb-1">Section Description</label>
                <input 
                  type="text" 
